...
| --- | --- |
| `pnpm dev` | Builds the library, then starts the site on port 3000 |
...
The site reads the library's built `dist/`, so rerun `pnpm --filter @tlannigan/crt build`
after changing a component. Playwright tests need `pnpm --filter web exec playwright install chromium` once.
